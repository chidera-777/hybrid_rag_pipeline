...
class Reranker:
    ...
    def rerank(self, query:str, chunks:List, top_k:int=3, return_scores:bool=False, threshold:float=None):
        if not chunks:
            return []
        ...
        if return_scores:
            return [scored_chunks[i] for i in range(top_k)]
        else:
            return [chunk for chunk, _ in scored_chunks][:top_k]
        
    def rerank_with_diversity(self, query:str, chunks:List, top_k:int=3, diversity_weight:float=0.3):
        if not chunks:
            return []
